...
mainbox {
    ...
    border-radius: 24px;
    border: 2px solid;
    border-color: $(hex_alpha "$ACCENT_PINK" "40");
    padding: 20px;
}
...
element selected {
    background-color: $(hex_alpha "$ACCENT_PINK" "26");
    border-color: $(hex_alpha "$ACCENT_PINK" "80");
}

element-icon {
    background-color: $(hex_alpha "$ACCENT_PINK" "19");
    border-radius: 14px;
    size: 256px;
}
...
mainbox {
    ...
    border-radius: 24px;
    border: 2px solid;
    border-color: $(hex_alpha "$ACCENT_CYAN" "40");
    padding: 20px;
...
inputbar {
    ...
    border-radius: 24px;
    border: 2px solid;
    border-color: $(hex_alpha "$ACCENT_TEAL" "59");
    padding: 14px 18px;
    margin: 0 4px 16px 4px;
    children: [prompt, entry];
    spacing: 10px;
...
element selected {
    background-color: $(hex_alpha "$ACCENT_CYAN" "26");
    border-color: $(hex_alpha "$ACCENT_CYAN" "80");
}

element-icon {
    background-color: $(hex_alpha "$ACCENT_CYAN" "19");
    border-radius: 14px;
    size: 256px;
}
...
generate_rofi_app() {
  cat >"$HOME/.config/rofi/style.rasi" <<EOF
...
* {
    ...
    fg1: ${FG1};
    accent: ${ACCENT_BLUE};
    accent-light: ${ACCENT_PURPLE};
    accent-lblue: ${ACCENT_LBLUE};
    transparent: #00000000;
}
...
inputbar {
    ...
    border-radius: 24px;
    border: 2px solid;
    border-color: $(hex_alpha "$ACCENT_PURPLE" "59");
    padding: 14px 18px;
    margin: 0 4px 16px 4px;
    children: [prompt, entry];
    spacing: 10px;
...
generate_all() {
  # Compute RGB components for BG0 (used by wlogout)
  hex_to_rgb "$BG0"
  BG0_R=$R
  BG0_G=$G
  BG0_B=$B
  ...
  generate_waybar
  # generate_rofi_app        # skipped — custom oxocarbon launcher in use
  # generate_rofi_wallpaper  # skipped — custom oxocarbon theme in use
  # generate_rofi_theme      # skipped — custom oxocarbon theme in use
  generate_wofi_app
  generate_wofi_wallpaper
  generate_wofi_theme
  generate_niri
...
main() {
  ...
  # Oxocarbon default thumbnail — use current wallpaper if swww is running
  local oxo_thumb="$THUMB_DIR/oxocarbon-default.png"
  local current_wall
  current_wall=$(swww query 2>/dev/null | grep -oP '(?<=image: ).*' | head -1)
