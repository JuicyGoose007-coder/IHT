...
STYLE="$HOME/.config/wofi/theme-switcher.css"
...
main() {
  ...
  entries+="img:${oxo_thumb}:text:Oxocarbon (Default)"$'\n'
  ...
  # Launch wofi
  local selection
  selection=$(printf '%s' "$entries" | wofi --dmenu \
    --prompt " Theme" \
    --style "$STYLE" \
    --allow-images \
    -D image_size=256 \
    ...
    --width 800 \
    --height 800 \
    --cache-file /dev/null) || true

  [[ -z "$selection" ]] && exit 0

  # Parse selection
  local theme_name
  theme_name=$(echo "$selection" | sed 's/^img:.*:text://')
  THEME_NAME="$theme_name"

  if [[ "$theme_name" == "Oxocarbon (Default)" ]]; then
    load_oxocarbon
  else
    extract_palette "$WALLPAPER_DIR/$theme_name"
    set_wallpaper "$WALLPAPER_DIR/$theme_name"
  fi
